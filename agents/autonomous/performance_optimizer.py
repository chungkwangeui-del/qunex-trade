...
class PerformanceOptimizer:
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> str:
        ...
        report.append("")
        report.append("Generated: " + str(now_str))
        report.append("")
        ...
        for issue_type, count in summary.get('by_type', {}).items():
            report.append("- " + str(issue_type.replace('_', ' ').title()) + ": " + str(count))
        ...
        for issue in self.issues[:20]:
            ...
            report.append("### [" + sev + "] " + str(issue.description))
            report.append("")
            report.append("**File:** " + str(issue.file_path) + " (line " + str(issue.line_number) + ")")
            report.append("")
            report.append("**Impact:** " + str(issue.estimated_impact))
            report.append("")
            report.append("**Suggestion:** " + str(issue.suggestion))
            report.append("")
            report.append("```python")
            report.append(str(issue.code_snippet))
            report.append("```")
            report.append("")
            report.append("---")
            report.append("")
