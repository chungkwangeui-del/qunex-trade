...
class TestAppInitialization:
    ...
    def test_database_models_defined(self):
        ...
        from web.database import (
            User, Watchlist, NewsArticle, EconomicEvent,
            AIScore, Transaction, BacktestJob
        )
...
class TestFreeAPIStrategy:
    ...
    def test_ai_score_uses_alpha_vantage(self):
        ...
        assert 'alpha_vantage.get_company_overview' in content or \
               'FundamentalData' in content, \
               "AI Score should use Alpha Vantage for fundamentals"

        # Verify Polygon Financials is NOT used for fundamentals
        assert 'polygon.get_financials' not in content and \
               'polygon.get_fundamental' not in content, \
    ...
    def test_calendar_uses_finnhub(self):
        ...
        assert 'finnhub.io' in content or 'FINNHUB_API_KEY' in content, \
        ...
        assert 'polygon' not in calendar_section.lower() or \
               'polygon.io' not in calendar_section, \
    ...
    def test_rate_limiting_in_ai_score(self):
        ...
        assert 'time.sleep' in content, \
               "AI Score should have rate limiting (time.sleep)"

        # Verify limiting to 20 stocks per run (5 calls/min limit)
        assert 'limit(20)' in content or 'limit=20' in content or ':20]' in content, \
