...
def main():
    ...
    parser.add_argument(
        "--symbols", nargs="+", default=None, help="Stock symbols (default: top 100 S&P 500)"
    )
    ...
    for symbol in tqdm(symbols, desc="Downloading data"):
        try:
            ...
            price_data = ai_model._fetch_historical_prices(
                symbol, args.start_date, args.end_date, polygon
            )

            if price_data is None or len(price_data) < 250:
                logger.warning(
                    f"Insufficient data for {symbol} (got {len(price_data) if price_data is not None else 0} days)"
                )
                failed += 1
                continue
            ...
            ):  # Need 200 days for features, 30 for forward return
                try:
    ...
    for symbol in test_symbols:
        try:
            ...
            recent_data = ai_model._fetch_historical_prices(
                symbol, "2023-01-01", "2024-12-31", polygon
            )
            ...
            if len(recent_data) >= 220:
                ...
                print(
                    f"✓ {symbol:6} AI Score: {score:3d}/100  |  Actual 20-day return: {actual_return:+6.2f}%"
                )
            else:
