    ...
    def calculate_confluence(
        ...
        if at_ob:
            ...
            if at_ob["type"] == "bullish":
                bullish_reasons.append({
                    "reason": "At Bullish Order Block" + (" (이중 장악형!)" if at_ob.get("is_double") else ""),
                    "strength": strength,
                })
            else:
                bearish_reasons.append({
                    "reason": "At Bearish Order Block" + (" (이중 장악형!)" if at_ob.get("is_double") else ""),
                    "strength": strength,
                })
...
- VWAP: ${vwap:.2f}
- Nearest Support: ${sr_levels['nearest_support']['price'] if sr_levels.get('nearest_support') else 'N/A'}
- Nearest Resistance: ${sr_levels['nearest_resistance']['price'] if sr_levels.get('nearest_resistance') else 'N/A'}
